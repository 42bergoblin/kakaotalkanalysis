        ...
        messages=[
            {
                "role": "system",
                "content": """
                너는 주요한 대화를 추출하는 역할을 맡았어.
                아래는 대화 내용이야. 이 중에서 주요한 내용을 추출해줘.
                각 대화는 누가 이야기했는지 구분되어서 추출되어야 해.
                """,
            },
            {
                "role": "user",
        ...
        messages=[
            {
                ...
                "content": """
                당신은 대화 분석 전문가임. 
                아래는 과거 대화 내용중 주요 내용을 추출한 것.
                이를 바탕으로 대화 분석 리포트를 작성.
        ...
        messages=[
            {
                ...
                전생은 사람이 아니라 동식물 혹은 물체일수도 있습니다.
                
                대화 내용을 기반으로 하되, 전생이기 때문에 최소 100년 전을 상상하여 관계를 각색하고 밀접한 관계를 만들어주세요.
                분석이 아닌 이야기 형태로 작성해주세요.
                이야기는 최소 5문단 이상, 500자 이상으로 작성해주세요. 
                """,
            },
            {
                "role": "user",
        ...
        messages=[
            {
                "role": "system",
                "content": """
                채팅 내역을 바탕으로 각 월의 가장 메인이 되는 이벤트를 명사형으로 추출하고, 그 이벤트에 대한 간략하 설명을 작성.
                최소 6개 이상의 이벤트를 추출.
                같은 월을 추출해도 되지만 이벤트 내용은 달라야함.
                일단위로 추출하지 말고 월단위로 추출. 
                같은 일자가 추출되면 안됨. (do not extract same date)

                결과 예시 :
                [n년 m월] : [이벤트명] - [이벤트 설명]
                [n년 m+1월] : [이벤트명2] - [이벤트 설명2]
                """,
            },
            {
                "role": "user",
